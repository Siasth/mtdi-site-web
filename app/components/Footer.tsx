...
export default function Footer({ locale: _locale, dict: _dict }: { locale?: string; dict?: any } = {}) {
  ...
  const [general, setGeneral] = useState<{
    logoFooter: string; siteName: string;
    facebook: string; twitter: string; linkedin: string; instagram: string; youtube: string;
  }>({
    logoFooter: "/mtdi-banner.png",
    siteName: "Ministère de la Transformation Digitale et de l'Innovation :République du Bénin",
  ...
  useEffect(() => {
    fetch("/api/general-settings")
      .then((r) => r.json())
      .then((d) => setGeneral((g) => ({
        logoFooter: d.logoFooter || g.logoFooter,
        siteName: d.siteName || g.siteName,
        facebook: d.facebook ?? g.facebook,
        twitter: d.twitter ?? g.twitter,
        linkedin: d.linkedin ?? g.linkedin,
        instagram: d.instagram ?? g.instagram,
        youtube: d.youtube ?? g.youtube,
      })))
      .catch(() => {});
  }, []);
  ...
  const socialLinks = (
    [
      ...
      { label: "X", href: general.twitter },
      { label: "LinkedIn", href: general.linkedin },
      { label: "Instagram", href: general.instagram },
      { label: "YouTube", href: general.youtube },
    ] as const
  )
    .filter((s) => s.href)
    .map((s) => ({ ...s, icon: SOCIAL_ICONS[s.label.toLowerCase() === "x" ? "twitter" : s.label.toLowerCase()] }));
