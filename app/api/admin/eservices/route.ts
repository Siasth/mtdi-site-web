...
export async function GET() {
  ...
  const result = await sql`SELECT * FROM eservices ORDER BY deleted_at NULLS FIRST, display_order ASC`;
...
export async function POST(req: NextRequest) {
  ...
  const { titleFr, titleEn, descriptionFr, descriptionEn, href, icon, displayOrder, active } = await req.json();
  if (!titleFr || !href) return NextResponse.json({ error: "Titre et lien sont requis" }, { status: 400 });
  const result = await sql`
    INSERT INTO eservices (title_fr, title_en, description_fr, description_en, href, icon_key, display_order, active, created_by)
    VALUES (${titleFr}, ${titleEn || null}, ${descriptionFr || null}, ${descriptionEn || null}, ${href}, ${icon || 'document'}, ${displayOrder ?? 0}, ${active ?? true}, ${session.id})
    RETURNING id
  `;
  await logAudit({ userId: session.id, action: "creer", module: "eservices", resourceId: String(result.rows[0].id), ip: getIp(req) });
