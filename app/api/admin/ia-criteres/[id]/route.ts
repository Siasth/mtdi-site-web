...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await sql`
    UPDATE ia_criteres SET
      text_fr = COALESCE(${textFr}, text_fr), text_en = ${textEn ?? null}, display_order = COALESCE(${displayOrder}, display_order), active = COALESCE(${active}, active)
    WHERE id = ${id}
  `;
  await logAudit({ userId: session.id, action: "modifier", module: "ia-criteres", resourceId: id, ip: getIp(req) });
  return NextResponse.json({ ok: true });
}
