...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.restore) {
    ...
    await sql`UPDATE opportunites SET deleted_at = NULL WHERE id = ${id}`;
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!hasPerm(session, "ressources.gerer")) return NextResponse.json({ error: "Permission refusée" }, { status: 403 });
  const { id } = await params;
  await sql`UPDATE opportunites SET deleted_at = now() WHERE id = ${id}`;
  await logAudit({ userId: session.id, action: "supprimer", module: "opportunites", resourceId: id, ip: getIp(req) });
  return NextResponse.json({ ok: true });
}
