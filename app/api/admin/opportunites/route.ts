...
export async function GET() {
  ...
  const result = await sql`SELECT * FROM opportunites ORDER BY deleted_at NULLS FIRST, display_order ASC`;
...
export async function POST(req: NextRequest) {
  ...
  const { type, titleFr, titleEn, descriptionFr, descriptionEn, href, deadline, displayOrder, active } = await req.json();
