...
export async function POST(req: NextRequest) {
  ...
  const { labelFr, labelEn, value, maxValue, unit, displayOrder, active } = await req.json();
  ...
  const result = await sql`
    INSERT INTO stats (label_fr, label_en, value, max_value, unit, display_order, active, created_by)
    VALUES (${labelFr}, ${labelEn || null}, ${value ?? 0}, ${maxValue ?? 100}, ${unit || ""}, ${displayOrder ?? 0}, ${active ?? true}, ${session.id})
    RETURNING id
  `;
