      ...
      {editP !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 py-8 overflow-y-auto">
          <form onSubmit={saveP} className="bg-white rounded-xl p-6 max-w-lg w-full shadow-2xl space-y-4 my-auto">
            ...
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="titre-fr">Titre (FR) *</label><input id="titre-fr" required value={pForm.titleFr} onChange={(e) => setPForm({ ...pForm, titleFr: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="titre-en">Titre (EN)</label><input id="titre-en" value={pForm.titleEn} onChange={(e) => setPForm({ ...pForm, titleEn: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
      ...
      {editJ !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <form onSubmit={saveJ} className="bg-white rounded-xl p-6 max-w-lg w-full shadow-2xl space-y-4">
            ...
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="annee">Année *</label><input id="annee" required type="number" inputMode="numeric" min={2000} max={2100} step={1} value={jForm.year} onChange={(e) => setJForm({ ...jForm, year: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="titre-fr-2">Titre (FR) *</label><input id="titre-fr-2" required value={jForm.titleFr} onChange={(e) => setJForm({ ...jForm, titleFr: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="titre-en-2">Titre (EN)</label><input id="titre-en-2" value={jForm.titleEn} onChange={(e) => setJForm({ ...jForm, titleEn: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="description-fr-2">Description (FR)</label><textarea id="description-fr-2" value={jForm.descriptionFr} onChange={(e) => setJForm({ ...jForm, descriptionFr: e.target.value })} rows={2} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="description-en-2">Description (EN)</label><textarea id="description-en-2" value={jForm.descriptionEn} onChange={(e) => setJForm({ ...jForm, descriptionEn: e.target.value })} rows={2} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
