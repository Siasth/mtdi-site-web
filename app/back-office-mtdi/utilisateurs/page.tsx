        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <form onSubmit={handleCreate} className="bg-white rounded-xl p-6 max-w-sm w-full shadow-2xl space-y-4">
            ...
            <div>
              ...
              <input id="nom-complet" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1" htmlFor="email">Email *</label>
              <input id="email" required type="email" pattern={EMAIL_PATTERN} title="Adresse email valide, ex : nom@domaine.fr" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
                className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1" htmlFor="role">Rôle *</label>
              <select id="role" required value={form.roleId} onChange={(e) => setForm({ ...form, roleId: e.target.value })}
                className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1" htmlFor="mot-de-passe-initial">Mot de passe initial *</label>
              <input id="mot-de-passe-initial" required type="text" minLength={12} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })}
                className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" />
          ...
          {resetSuccess ? (
            ...
            <form onSubmit={handleResetPassword} className="bg-white rounded-xl p-6 max-w-sm w-full shadow-2xl space-y-4">
              ...
              <div>
                ...
                <input id="nouveau-mot-de-passe" required type="text" minLength={12} value={resetPassword} onChange={(e) => setResetPassword(e.target.value)}
