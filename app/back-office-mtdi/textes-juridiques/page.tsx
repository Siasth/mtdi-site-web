...
export default function AdminTextesJuridiques() {
  ...
  function load() { setLoading(true); fetch("/api/admin/textes-juridiques").then((r) => r.json()).then((d) => { setItems(d); setLoading(false); }); }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 py-8 overflow-y-auto">
          <form onSubmit={handleSave} className="bg-white rounded-xl p-6 w-[90%] max-w-2xl shadow-2xl space-y-4 my-auto">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1">Date</label><input value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
            </div>
