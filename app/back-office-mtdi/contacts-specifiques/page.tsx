  ...
  return (
    <div className="p-8">
      ...
      <div className="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100">
        ...
        {visibleItems.length === 0 && <p className="p-8 text-center text-gray-400">{showDeleted ? "Aucun élément supprimé." : "Aucun contact"}</p>}
      </div>
      ...
      {editing !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <form onSubmit={handleSave} className="bg-white rounded-xl p-6 max-w-lg w-full shadow-2xl space-y-4">
            ...
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="note">Note</label><textarea id="note" value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} rows={2} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="couleur">Couleur</label><input id="couleur" type="color" value={form.accent} onChange={(e) => setForm({ ...form, accent: e.target.value })} className="w-20 h-9 rounded border border-gray-200" /><ColorContrastHint color={form.accent} /></div>
