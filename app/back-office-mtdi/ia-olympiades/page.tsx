      ...
      {editE !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 py-8 overflow-y-auto">
          <form onSubmit={saveE} className="bg-white rounded-xl p-6 max-w-lg w-full shadow-2xl space-y-4 my-auto">
            ...
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="annee">Année *</label><input id="annee" required type="number" inputMode="numeric" min={2000} max={2100} step={1} value={eForm.year} onChange={(e) => setEForm({ ...eForm, year: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="titre-fr">Titre (FR) *</label><input id="titre-fr" required value={eForm.titleFr} onChange={(e) => setEForm({ ...eForm, titleFr: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="titre-en">Titre (EN)</label><input id="titre-en" value={eForm.titleEn} onChange={(e) => setEForm({ ...eForm, titleEn: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="description-fr">Description (FR)</label><textarea id="description-fr" value={eForm.descriptionFr} onChange={(e) => setEForm({ ...eForm, descriptionFr: e.target.value })} rows={2} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="description-en">Description (EN)</label><textarea id="description-en" value={eForm.descriptionEn} onChange={(e) => setEForm({ ...eForm, descriptionEn: e.target.value })} rows={2} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="point-marquant-fr">Point marquant (FR)</label><input id="point-marquant-fr" value={eForm.highlightFr} onChange={(e) => setEForm({ ...eForm, highlightFr: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="point-marquant-en">Point marquant (EN)</label><input id="point-marquant-en" value={eForm.highlightEn} onChange={(e) => setEForm({ ...eForm, highlightEn: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
      ...
      {editC !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <form onSubmit={saveC} className="bg-white rounded-xl p-6 max-w-lg w-full shadow-2xl space-y-4">
            ...
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="texte-fr">Texte (FR) *</label><textarea id="texte-fr" required value={cForm.textFr} onChange={(e) => setCForm({ ...cForm, textFr: e.target.value })} rows={2} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
            <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1" htmlFor="texte-en">Texte (EN)</label><textarea id="texte-en" value={cForm.textEn} onChange={(e) => setCForm({ ...cForm, textEn: e.target.value })} rows={2} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" /></div>
