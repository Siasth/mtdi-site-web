...
export default function MTDIDansLesMediasPage() {
  ...
  const t = locale === "en" ? en.medias : fr.medias;

  const [activeFilter, setActiveFilter] = useState(FILTER_ALL);

  const filters = [
    { value: FILTER_ALL, label: t.filtres.tous },
    { value: "Médias", label: t.filtres.medias },
  ];
            ...
            <div className="flex gap-px overflow-x-auto" style={{ scrollbarWidth: "none" }}>
              {filters.map((filter) => (
                <button
                  ...
                  style={{
                    background: activeFilter === filter.value ? VERT : "rgba(0,0,0,0.04)",
                    color: activeFilter === filter.value ? "white" : "rgba(26,26,26,0.45)",
                  }}
                >
                  {filter.label}
                </button>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start gap-4 sm:gap-8">
                    <div className="flex-shrink-0 flex sm:flex-col items-center sm:items-start gap-3 sm:gap-2 sm:w-44">
                      <span
                        ...
                        style={{ background: item.typeColor, color: item.typeText }}
                      >
                        {item.type}
                      </span>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center justify-between pt-4" style={{ borderTop: "1px solid rgba(0,0,0,0.06)" }}>
                        <span className="text-anthracite/60 text-[10px] font-semibold uppercase tracking-wider">
                          {item.source}
                        </span>
                        <span className="text-xs font-black uppercase tracking-widest transition-all" style={{ color: VERT }}>
                          {t.voir}
                        </span>
                      </div>
                    </div>
