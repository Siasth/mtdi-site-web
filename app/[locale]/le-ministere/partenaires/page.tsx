...
export default async function PartenairesPage({ params }: Props) {
  ...
  // ANO-113/ANO-156 : cette page affichait une liste de partenaires codée en
  // dur dans le code, totalement déconnectée du back-office — un partenaire
  // créé, modifié ou dépublié depuis l'interface d'administration n'avait
  // donc jamais le moindre effet sur le site. getPartners() lit la vraie
  // table (et exclut déjà les partenaires supprimés ou désactivés).
  const allPartners = await getPartners(locale === "en" ? "en" : "fr");

  const categories = [
    { label: "Partenaires institutionnels", id: "institutionnels", accent: "#006828", partners: allPartners.filter((p) => p.category === "institutionnel") },
    { label: "Partenaires technologiques & internationaux", id: "technologiques", accent: "#A07800", partners: allPartners.filter((p) => p.category === "technologique") },
    { label: "Partenaires académiques", id: "academiques", accent: ROUGE, partners: allPartners.filter((p) => p.category === "academique") },
  ];
              ...
              {category.partners.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-px" style={{ background: "rgba(0,0,0,0.08)" }}>
                  {category.partners.map((partner) => (
                    <div key={partner.id} className="group p-8 bg-white hover:bg-gris-perle transition-colors">
